...
- 배포 URL: `여기에 Vercel 배포 후 URL을 적으세요 (예: https://red-study-mystery.vercel.app)`
- GitHub 저장소: `여기에 저장소 URL을 적으세요`
...
- **AI API**: [Groq API](https://console.groq.com) — `llama-3.3-70b-versatile` 모델 (무료 티어)
...
3. Framework Preset은 `Other`(정적 사이트)로 두고 그대로 Deploy 한다.
4. 배포가 끝나면 Vercel 프로젝트의 **Settings → Environment Variables**에서
   `GROQ_API_KEY` 를 등록하고(값은 아래 "API 키 발급" 참고) **Redeploy** 한다.
...
├─ vercel.json           # Vercel Python 런타임 설정
├─ PLAN.md              # 서비스 기획서
└─ README.md
```
...
`iPhone SE`, `iPad Air` 두 가지 화면 크기에서 직접 확인하는 것을 권장합니다.
